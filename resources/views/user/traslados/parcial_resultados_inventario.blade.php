        ...
        <tbody>
            @forelse($detalles as $detalle)
                ...
                <tr data-id-activo="{{ $idActivo }}">
                    ...
                    <td>
                        {{-- {{ $detalle->en_otras_traslados }} --}}
                        @if ($detalle->en_traslado_actual ?? false)
                            <button class="btn btn-sm btn-outline-danger btn-eliminar-activo"
                                data-id-activo="{{ $idActivo }}"
                                data-id-traslado="{{ $detalle->id_traslado_actual }}">
                                Eliminar
                            </button>
                        @elseif($detalle->en_otros_traslados ?? false)
...
<script>
    $(document).ready(function() {
        ...
        $(document).off('click', '.btn-ver-detalle').on('click', '.btn-ver-detalle', function() {
            ...
            // Creamos los botones tal como estaban, solo agregando data-id-activo y data-id-traslado
            let botonesHTML = '';
            actas.forEach(acta => {
                botonesHTML += `
            <button type="button" id="seleccionar_traslado" class="btn btn-sm btn-outline-secondary btn-seleccionar-traslado"
                data-id-activo="${idActivo}" data-id="${acta.id_traslado}">
                Revisar esta acta
            </button>
            <button type="button" class="btn btn-sm btn-danger btn-eliminar-activo"
                data-id-activo="${idActivo}" data-id-traslado="${acta.id_traslado}">
